...
def extract_pdf_data(pdf_file_path):
    ...
    tabula.convert_into(pdf_file_path, "output.csv", output_format="csv", pages='all') 
    ...
    ## create new three columns date , month and year extracting from Date of\rEncashment

    df['Date'] = df['Date of\rEncashment'].str.split('/').str[0].astype(int) ### stuck here for 2 hr just .astype(int)
